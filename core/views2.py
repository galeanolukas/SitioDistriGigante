...
from .forms import UserForm, PerfilForm, MercadoPagoCuentaForm, ProductForm, CartelForm, SuperuserCreationForm, CargarExcelForm, UbicacionForm, AgregarProductoForm, RegistrationForm
...
def detalle_pedido(request, pedido_id):
    ...
    if request.user.is_superuser:
        ...
        # Si es un usuario regular, solo permite ver sus propios pedidos
...
def pedidos_panel(request):
    ...
    # Estadísticas
    pedidos_entregados = pedidos_list.filter(estado='entregado').count()
    pedidos_pendientes = pedidos_list.filter(estado='pendiente').count()
    pedidos_pagados = pedidos_list.filter(estado='pagado').count()
...
def carrito(request):
    ...
    if es_vendedor or es_administrador:
        # Obtener perfiles de clientes en lugar de usuarios
...
def product_detail(request, product_id):
    ...
    request.META['OG_TITLE'] = f"{product.nombre} - Distribuidora Gigante"
...
def panel_gestion_clientes(request):
    ...
    if es_administrador:
        ...
        todos_vendedores = Perfil.objects.filter(
            tipo_usuario__in=['vendedor', 'admin']
        ).select_related('user')

        # Incluir también superusuarios
        superusers = User.objects.filter(is_superuser=True)
        for superuser in superusers:
            perfil_superuser, created = Perfil.objects.get_or_create(
                user=superuser,
                defaults={'tipo_usuario': 'admin'}
            )
            if perfil_superuser not in todos_vendedores:
                todos_vendedores = todos_vendedores | Perfil.objects.filter(id=perfil_superuser.id)
...
def generar_pdf_pedido(request, pedido_id):
    try:
        ...
        html_string = render_to_string('pdf_pedido.html', context)
